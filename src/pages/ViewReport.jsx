...
export default function ViewReport({ reportId, onClose }) {
  ...
  // Fetch the full report data when the modal opens
  useEffect(() => {
    if (!reportId) return;
    let cancel = false;
    (async () => {
  ...
  // Generate outputs whenever the report data changes
  useEffect(() => {
    if (!report) return;
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                ...
                <MetadataRow label="Created On" value={new Date(report.created_on).toLocaleString()} />
...
// Sub-components for styling and organization
function ClassificationBanner({ classification }) {
  ...
  return (
    <div className={`text-white text-center font-bold text-sm py-1 rounded ${styles[classification] || 'bg-gray-500'}`}>
      {classificationForOutput(classification)}
    </div>
  );
}
...
function MetadataRow({ label, value }) {
  return (
    <div className="grid grid-cols-3 gap-2 text-sm">
      <span className="font-semibold text-slate-400 col-span-1">{label}:</span>
  ...
  return (
    <div>
      ...
      <textarea
        readOnly
        value={text}
        className="w-full text-xs min-h-[100px] rounded-md bg-slate-900 border border-slate-700 px-3 py-2"
      />
      <button
        type="button"
        className="w-full mt-2 h-8 rounded-md text-sm bg-slate-700 hover:bg-slate-600 border border-green-400 text-green-400"
        onClick={() => onCopy(text)}
      >
        Copy {label}
      </button>
